...
!!
 subroutine cu_ntiedtke_pre_run(its,ite,kts,kte,kte_plus_one,itimestep,stepcu,dt,grav,dz,pres,presi,   &
                                t,rho,qv,qc,qi,u,v,w,qvften,thften,qvftenz,thftenz,slimsk,prsl,ghtl, &
                                tf,qvf,qcf,qif,uf,vf,prsi,ghti,omg,errmsg,errflg)
!=================================================================================================================

!--- input arguments:
 integer,intent(in):: its,ite,kts,kte,kte_plus_one
 integer,intent(in):: itimestep
 integer,intent(in):: stepcu

 real(kind=kind_phys),intent(in):: dt,grav
 real(kind=kind_phys),intent(in),dimension(:,:):: dz,pres,t,rho,qv,qc,qi,u,v
 real(kind=kind_phys),intent(in),dimension(:,:):: qvften,thften
 real(kind=kind_phys),intent(in),dimension(:,:):: presi,w

!--- inout arguments:
! integer,intent(inout):: im,kx,kx1
 integer,intent(inout),dimension(:):: slimsk

!--- output arguments:
! real(kind=kind_phys),intent(out):: delt
 real(kind=kind_phys),intent(out),dimension(:,:):: qvf,qcf,qif,uf,vf,tf
 real(kind=kind_phys),intent(out),dimension(:,:):: prsl
 real(kind=kind_phys),intent(out),dimension(:,:):: qvftenz,thftenz
 real(kind=kind_phys),intent(out),dimension(:,:):: ghtl,omg
 real(kind=kind_phys),intent(out),dimension(:,:):: ghti,prsi
...
!--- local variables and arrays:
 ...
 real(kind=kind_phys),dimension(its:ite, kts:kte_plus_one):: zi
